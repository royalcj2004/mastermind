class Messages
	...
	def self.mastermind_evaluation_status(evaluation_hash)
		...
		#correct_code = game_status_hash[:computer_secret_code]
		#times_tried = game_status_hash[:times_tried]
		#times_left = game_status_hash[:times_left]
		exact_matches = evaluation_hash[:exact]
		partial_matches = evaluation_hash[:partial]

		"You had #{exact_matches.to_s.green} positions correctly matched and #{partial_matches.to_s.yellow} close matches."
	end


=begin
You have tried #{times_tried.to_s.cyan} time(s). You have #{times_left.to_s.magenta} attempts left.
=end


	def self.correct_code_message (computer_secret_code_array)
		"Congratulations!!! You guessed right. The secret code is #{computer_secret_code_array}"
	end


	def self.mastermind_too_short
		"Entry is too short. Try again!!!"
	end

	def self.mastermind_too_long
		"Entry is too long. Try again!!!"
	end
